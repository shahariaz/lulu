...
**Document Status:** Revision 2 — Resolved Architectural Contracts & Open Choices  
**Date:** 2026-09-09  
...
| **`DEC-03`** | **Execution Security Distinct from Git** | Working directory confinement (`assertPathWithinWorktree`), environment credential scrubbing, process tree timeouts (`setpgid`), and command allowlists provide a cooperative runtime boundary on the host, not an impenetrable hypervisor sandbox. An advanced containerized tier is defined as a separate proposal. |
...
| **`DEC-06`** | **Specialist Review Availability Rule** | If the configured specialist reviewer model is unavailable or rate-limited, the task remains pending in `Code Review` with blocker reason `SPECIALIST_UNAVAILABLE`. Review must never be downgraded to a worker tier or bypassed without explicit owner override. |
...
2. **Worker Harness Integration Strategy (Spike-First):**
   ...
   *Rationale:* If the CLI passes mock gateway handshake tests, it provides mature coding and diffing tools. If it fails on headers or flags, Approach C provides complete protocol control.
...
- **Options:**
  - **Option A (Recommended):** Execute `TSK-SPIKE-HARNESS-PARITY` (`TSK-M1-05`) as an offline gating spike. If the CLI passes the mock test, implement Approach A (Headless CLI); if it fails, implement Approach C (Custom In-Process Loop).
  - **Option B:** Commit immediately to Approach C (Custom Node.js Loop) without running the spike, avoiding external CLI binary execution entirely.
