...
export class AntigravityAccountManager {
  ...
  saveToConfigFile(account) {
    try {
      ...
      // `source` must be 'oauth': the gateway serializes refreshToken as
      // `acc.source === 'oauth' ? acc.refreshToken : undefined`, so an entry
      // without it loses its refresh token the next time the gateway saves,
      // and then falls through to reading the local Antigravity IDE database.
      const accEntry = {
        email,
        source: 'oauth',
  ...
  // The gateway that actually serves traffic builds its rotation pool only from
  // AGW_CONFIG_PATH, while the dashboard lists accounts from the SQLite DB. Any
  // account that reaches the DB without also reaching the config file is
  // invisible to the router, so the pool silently shrinks and requests keep
  // piling onto the accounts the file happens to know about. Mirror the DB into
  // the file on every load so the two cannot drift apart.
  syncConfigFile() {
    try {
      const dir = path.dirname(AGW_CONFIG_PATH)
      if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
      ...
      const byEmail = new Map(config.accounts.map((entry) => [entry.email, entry]))
      const added = []
      for (const acc of this.accounts) {
        if (acc.status === 'invalid') continue
        // Without a refresh token the gateway cannot mint a bearer, so the entry
        // would just be a dead slot in the rotation.
        if (!acc.refresh_token) continue
        const existing = byEmail.get(acc.email)
        // Spread the existing entry first so gateway-owned fields the DB does
        // not track (modelRateLimits, lastUsed, isInvalid) survive the write.
        const entry = {
          ...(existing || {}),
          email: acc.email,
          // Must be 'oauth'. The gateway writes the config back as
          // `refreshToken: acc.source === 'oauth' ? acc.refreshToken : undefined`,
          // so an entry missing this loses its refresh token on the gateway's
          // next save and then falls back to reading the local Antigravity IDE
          // database, which fails with a 500 for every request it serves.
          source: existing?.source || 'oauth',
          projectId: acc.account_id || existing?.projectId || DEFAULT_PROJECT_ID,
          refreshToken: acc.refresh_token,
          addedAt: existing?.addedAt || acc.created_at || Date.now(),
          isInvalid: false,
          invalidReason: null,
        ...
        if (existing) {
          config.accounts[config.accounts.indexOf(existing)] = entry
        } else {
          config.accounts.push(entry)
          byEmail.set(acc.email, entry)
          added.push(acc.email)
        }
      }

      if (!(config.activeIndex >= 0 && config.activeIndex < config.accounts.length)) {
        config.activeIndex = 0
      }
      fs.writeFileSync(AGW_CONFIG_PATH, JSON.stringify(config, null, 2))
      if (added.length > 0) {
        console.warn(`[AntigravityAccounts] Added ${added.length} DB-only account(s) to the gateway pool: ${added.join(', ')}`)
      }
      return added
    } catch (e) {
      console.error('[AntigravityAccounts] Failed to sync config file:', e.message)
      return []
    }
  }
