...
import { sweepProviderContracts, formatDrift } from './provider-contract.mjs'
...
/**
 * Where the compiled React app lives.
 *
 * Resolved per call rather than once at module load so ZEN_STATIC_DIR can be set by a caller or
 * a test after import. It is also why the UI tests can be hermetic: dist/ is gitignored, so
 * asserting against a real build made them pass only for someone who had run `npm run build:ui`
 * and fail on every fresh clone and in CI.
 */
function resolveFrontendDist() {
  return process.env.ZEN_STATIC_DIR
    ? path.resolve(process.env.ZEN_STATIC_DIR)
...
export function startOrchestratorServer({
  ...
  // Provider contract check, in the background so boot is not blocked on a third party.
  //
  // Contract tests prove the gateways' behaviour at the moment they run. This is what covers
  // the hours in between: providers change underneath us (one gateway here silently answers a
  // request for `claude-zen-opus` with `gpt-5.6-sol@low`), and without a recorded baseline that
  // change is invisible until something downstream produces bad work. Skips gateways whose last
  // snapshot is still fresh, so frequent restarts do not spend quota to learn nothing.
  if (!skipRecovery && process.env.ZEN_SKIP_CONTRACT_SWEEP !== '1') {
    sweepProviderContracts({
      db: targetDb,
      onDrift: ({ target, diff }) => {
        console.warn(`[Contract] ${formatDrift(target.gatewayUrl, diff)}`)
        broadcastOrchestratorEvent('provider_contract_drift', { gatewayUrl: target.gatewayUrl, changes: diff.changes })
      },
    }).catch((err) => console.error('[Contract] Sweep failed (continuing):', err.message))
  }
