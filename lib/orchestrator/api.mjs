...
export function createOrchestratorHandler({ db = null } = {}) {
  ...
  return async function handleOrchestratorRequest(req, res) {
    ...
    try {
      ...
      if (staticDir && (req.method === 'GET' || req.method === 'HEAD') && !pathname.startsWith('/api/')) {
      ...
      // -------------------------------------------------------------
      // Product Design Council & Idea-to-Product Studio
      // -------------------------------------------------------------
      if (req.method === 'POST' && pathname === '/api/orchestrator/council/start') {
        const body = await readJsonBody(req)
        const session = startCouncilSession({
          projectName: body.projectName || '',
          ideaDescription: body.ideaDescription || '',
          targetPersona: body.targetPersona || 'developers',
        })
        return sendJson(res, 200, { session, personas: COUNCIL_PERSONAS })
      }

      if (req.method === 'POST' && pathname === '/api/orchestrator/council/turn') {
        const body = await readJsonBody(req)
        const result = await executeCouncilTurn({
          sessionId: body.sessionId,
          userMessage: body.userMessage,
          forcedRole: body.forcedRole || null,
          mockReply: body.mockReply || null,
        })
        return sendJson(res, 200, result)
      }

      if (req.method === 'POST' && pathname === '/api/orchestrator/council/teardown') {
        const body = await readJsonBody(req)
        const teardown = generateCompetitorTeardown({
          productIdea: body.productIdea,
          industryCategory: body.industryCategory,
          knownCompetitors: body.competitors || [],
        })
        return sendJson(res, 200, { teardown })
      }

      if (req.method === 'POST' && pathname === '/api/orchestrator/council/blueprint') {
        const body = await readJsonBody(req)
      ...
      if (req.method === 'POST' && pathname === '/api/orchestrator/council/initialize-project') {
        const body = await readJsonBody(req)
        const { repoPath, projectName, blueprint } = body
        if (!repoPath) throw new Error('repoPath is required')
        ...
        // 2. Register project
        const project = createProject({
          name: projectName || blueprint?.title || path.basename(resolvedPath),
          repoPath: resolvedPath,
          activeBranch: 'main',
        }, targetDb)

        // 3. Create initial README and package.json if empty
        const readmePath = path.join(resolvedPath, 'README.md')
        if (!fs.existsSync(readmePath)) {
          fs.writeFileSync(readmePath, `# ${project.name}\n\n${blueprint?.prd?.executiveSummary || 'Engineered with Claude-Zen Autonomous Studio.'}\n`)
          runGit(resolvedPath, ['add', 'README.md'])
          runGit(resolvedPath, ['commit', '-m', 'chore: initialize project repository from blueprint'])
        }

        // 4. Create and approve Baseline v1.0.0 from Blueprint
        const specMarkdown = `# Product Requirements Document: ${project.name}
Version: v1.0.0
Author: Claude-Zen Product Council

## 1. Executive Summary
${blueprint?.prd?.executiveSummary || 'Requirements definition from Product Council.'}

## 2. Market & Unique Differentiators
${(blueprint?.market?.uniqueDifferentiators || []).map((d) => '- ' + d).join('\n')}

## 3. Functional Requirements
${(blueprint?.prd?.requirements || []).map((r) => `- ${r.id}: ${r.title} (${r.priority})`).join('\n')}

## 4. Architecture & Database Schema
${blueprint?.architecture?.techStack || 'Node.js 24 + TypeScript'}
`
        ...
        // 5. Hierarchical Decomposition into Epics and Tasks
        const epicsWithTasks = (blueprint?.roadmap?.milestones?.[0]?.tasks || []).length > 0
          ? [
              {
                title: 'Epic 1: Foundation & Core Engine',
                description: 'MVP Architecture and Domain Entities',
                tasks: blueprint.roadmap.milestones[0].tasks.map((t, idx) => ({
                  tempId: `t_${idx + 1}`,
                  title: t.title,
                  scopePaths: t.scopePaths || ['src/app.js'],
                  blockedBy: idx > 0 ? [`t_${idx}`] : [],
                  linkedRequirementIds: [`REQ-F-0${idx + 1}`],
                })),
              },
            ]
          : []
        ...
        if (epicsWithTasks.length > 0) {
          decomposition = decomposeBaselineHierarchical({
            baselineId: approved.id,
            projectId: project.id,
            milestoneTitle: 'Milestone 1: MVP Delivery',
            epicsWithTasks,
          }, targetDb)
        }
        ...
        broadcastOrchestratorEvent('project_initialized_from_blueprint', {
          ...
          taskCount: decomposition?.tasks?.length || 0,
        })
      ...
      if (req.method === 'POST' && scopeStartMatch) {
        ...
        if (body.initialPrompt && body.initialPrompt.trim()) {
          await generateArchitectReply(session.id)
        }
        return sendJson(res, 200, { session: getSpecConversation(session.id) })
      }
      ...
      if (req.method === 'POST' && scopeMsgMatch) {
        ...
        const body = await readJsonBody(req)
        addSpecMessage(conversationId, {
          role: body.role || 'user',
          content: body.content,
        })
        const replyResult = await generateArchitectReply(conversationId)
        return sendJson(res, 200, { session: replyResult.conversation, reply: replyResult.message })
      }
