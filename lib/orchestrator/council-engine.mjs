 ...
 */
async function refreshResearchCache({ ideaTitle, ideaDescription }) {
  ...
  try {
    ...
    await spawnControlledProcess(
      'wigolo',
      ['search', keywords, '--json', '--force-refresh', '--search-depth=fast'],
      { timeoutMs: 45000 },
    )
  } catch (err) {
...
export async function researchMarket({
  ...
  try {
    ...
    if (!runner) await refreshResearchCache({ ideaTitle, ideaDescription })
    let processResult
    try {
