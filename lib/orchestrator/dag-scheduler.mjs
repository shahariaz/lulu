...
export const AUTONOMY_MODES = Object.freeze(['GUIDED', 'SUPERVISED', 'AUTONOMOUS'])

/** Decide whether the orchestrator may cross a task boundary without an owner click. */
export function isTransitionAutoAllowed(mode, fromStatus, toStatus) {
  if (!AUTONOMY_MODES.includes(mode)) return false
  if (fromStatus === 'Ready' && toStatus === 'In Progress') {
    return mode === 'SUPERVISED' || mode === 'AUTONOMOUS'
  }
  if (fromStatus === 'QA' && toStatus === 'Done') {
    return mode === 'AUTONOMOUS'
  }
  return true
}

function getTaskProjectAutonomyMode(taskId, db) {
  return db.prepare(`
    SELECT p.autonomy_mode
    FROM tasks t
    JOIN milestones m ON m.id = t.milestone_id
    JOIN requirements_baselines b ON b.id = m.baseline_id
    JOIN projects p ON p.id = b.project_id
    WHERE t.id = ?
  `).get(taskId)?.autonomy_mode || 'GUIDED'
}
 ...
 */
// Signature note: `db` stays in third position. Moving it would make the seven existing call
// sites pass their test database as the options object, silently falling back to the real
// ~/.zen-claude database — a test-suite footgun worse than an awkward parameter order.
export function claimTaskForExecution(taskId, projectId, db = null, { automatic = false } = {}) {
  const targetDb = db || getOrchestratorDb()
  const task = getTask(taskId, targetDb)
  if (!task) throw new Error(`Task not found: ${taskId}`)
  ...
  // Ready → In Progress is one of the two boundaries the autonomy dial governs. This function
  // used to call updateTaskStatus directly, so the gate was enforced only in unit tests while
  // the real /claim endpoint bypassed it — GUIDED behaved like SUPERVISED the moment anything
  // dispatched work without a human click.
  if (automatic) {
    const mode = getTaskProjectAutonomyMode(taskId, targetDb)
    if (!isTransitionAutoAllowed(mode, 'Ready', 'In Progress')) {
      throw new IllegalStateTransitionError(
        taskId, task.status, 'In Progress',
        `${mode} mode requires owner approval before a task starts`,
      )
    }
  }

  // The lock check and the status write must be one atomic step. As two statements they were a
  // TOCTOU: two concurrent claims for different tasks in the same project could both observe an
  // unlocked workspace and both proceed, defeating the single-active-writer invariant this
...
export function transitionTask(taskId, targetStatus, {
  ...
  if (task.status !== targetStatus) {
    ...
    if (automatic) {
      const autonomyMode = getTaskProjectAutonomyMode(taskId, targetDb)
      if (!isTransitionAutoAllowed(autonomyMode, task.status, targetStatus)) {
        throw new IllegalStateTransitionError(taskId, task.status, targetStatus, `${autonomyMode} mode requires owner approval`)
      }
    }
  }
