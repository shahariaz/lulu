 ...
 *
 * Runs BEFORE the HTTP server accepts connections, so the synchronous grace-wait inside
 * terminateProcessGroup is harmless here — there is no traffic to block yet. (That wait is a
 * real problem for runtime callers such as WorkerPoolManager.shutdownPool, which invokes it
 * serially per worker on the main thread; that path is not wired up yet and is tracked
 * separately.)
 */
export function performStartupRecovery(db = null, { gitExecutor = null } = {}) {
  ...
  //
  // Only 'RUNNING' and 'PENDING' are reachable: the task_runs.status CHECK constraint permits
  // PENDING|RUNNING|SUCCEEDED|FAILED|CANCELLED|INTERRUPTED. This query previously also listed
  // 'VERIFYING' and 'REVIEWING', which the constraint rejects on write — dead branches that
  // read like coverage the sweep did not actually have.
