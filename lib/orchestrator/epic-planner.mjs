...
export function decomposeBaselineHierarchical({
  ...
  // 1. Create Milestones (preserve multiple roadmap phases if provided)
  const createdMilestones = []
  if (Array.isArray(milestones) && milestones.length > 0) {
    for (let i = 0; i < milestones.length; i++) {
      const msDef = milestones[i]
      const msTitle = typeof msDef === 'string' ? msDef : (msDef.title || `Milestone ${i + 1}`)
      const orderIndex = typeof msDef === 'object' && msDef.orderIndex ? msDef.orderIndex : i + 1
    ...
    const defaultMs = createMilestone({
      baselineId,
      title: milestoneTitle,
      orderIndex: 1,
    }, targetDb)
    createdMilestones.push(defaultMs)
  }
  const primaryMilestone = createdMilestones[0]
  ...
  for (let i = 0; i < epicsWithTasks.length; i++) {
    ...
    createdEpics.push({ epic, epicDef, featureDefs, epicIndex: i })
  }

  // 3. Create Features and Tasks (Pass 1: Insert records)
  const pendingDependencyTasks = []

  for (const { epic, epicDef, featureDefs, epicIndex } of createdEpics) {
    for (let featureIndex = 0; featureIndex < featureDefs.length; featureIndex++) {
      ...
      for (const tDef of featureDef.tasks || []) {
        ...
        // Map task to corresponding milestone if multi-milestone roadmap exists
        const milestoneTarget = tDef.milestoneIndex !== undefined && createdMilestones[tDef.milestoneIndex]
          ? createdMilestones[tDef.milestoneIndex]
          : epicDef.milestoneIndex !== undefined && createdMilestones[epicDef.milestoneIndex]
          ? createdMilestones[epicDef.milestoneIndex]
          : (createdMilestones.length > 1 && epicIndex < createdMilestones.length)
          ? createdMilestones[epicIndex]
          : primaryMilestone

        const task = createTask({
          milestoneId: milestoneTarget.id,
          title: tDef.title,
  ...
  // 5. Automatically unblock eligible initial tasks in each milestone
  for (const ms of createdMilestones) {
    resolveNextReadyTasks(ms.id, targetDb)
  }
  ...
  return {
    ...
    tasks: createdMilestones.flatMap((ms) => listTasks(ms.id, targetDb)),
  }
}
