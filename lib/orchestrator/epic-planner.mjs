...
function parseDecompositionJson(text, gatewayUrl) {
  ...
  try {
    ...
    throw new GatewayUnavailableError(`Planning gateway returned malformed JSON: ${err.message}`, { gatewayUrl, cause: err })
  }
}
...
export async function decomposePRDViaLLM({
  ...
  if (!candidate) {
    ...
    const response = gatewayRunner
      ...
      : await callGatewayForText({ gatewayUrl, model, system: HIERARCHICAL_DECOMPOSITION_PROMPT, messages, maxTokens: 8192 })
    candidate = typeof response === 'string' ? parseDecompositionJson(response, gatewayUrl) :
      response?.text ? parseDecompositionJson(response.text, gatewayUrl) : response
  }
