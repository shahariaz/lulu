...
import {
  createProject, getBaseline, updateConversation,
} from '../../db/index.mjs'
import { runGit } from '../../git-workspace.mjs'
import {
  startCouncilSession, executeCouncilTurn, generateCompetitorTeardown,
  synthesizeProductBlueprint, validateProductBlueprint, getCouncilSession,
  COUNCIL_PERSONAS,
...
import { decomposePRDViaLLM, decomposeBaselineHierarchical } from '../../epic-planner.mjs'
...
export function registerProductDesignRoutes(app, { db }) {
  ...
  app.post('/api/orchestrator/council/initialize-project', async (c) => {
    ...
    const decomposition = decomposeBaselineHierarchical({ baselineId: baseline.id, projectId: project.id, milestoneTitle: planned.milestoneTitle || 'Milestone 1: MVP Delivery', epicsWithTasks: planned.epics }, db)
    if (sessionId && session) updateConversation(sessionId, { projectId: project.id }, db)
    broadcastOrchestratorEvent('project_initialized_from_blueprint', { projectId: project.id, projectName: project.name, baselineVersion: baseline.version, taskCount: decomposition.tasks.length })
    return json(c, { success: true, project, baseline, decomposition }, 201)
  })
