import { observeEnvelope } from './provider-contract.mjs'
/**
...
/**
 * Token budget for any call whose response is PARSED rather than read.
 *
 * max_tokens is shared between reasoning and output on these models, so a structured request on
 * a chat-sized budget starves its own answer and returns JSON cut off mid-array — while
 * reporting `stop_reason: "end_turn"`. Measured on this gateway: 7,860 tokens of reasoning left
 * 328 for the answer.
 *
 * The blueprint and decomposition calls were each raised individually after being caught in
 * production; market-research structuring and the QA verdict were missed both times, because
 * nothing tied "this output is parsed" to "this needs a real budget". This constant is that tie.
 * A prose reply the caller reads as text does not need it.
 */
export const STRUCTURED_OUTPUT_MAX_TOKENS = Number(process.env.ZEN_STRUCTURED_MAX_TOKENS || 32000)
...
export async function callGatewayForText({
  ...
  // Observe the contract on EVERY real call, not just on a cadence. This is what makes provider
  // drift visible at the moment it affects work rather than at the next test run: the caller
  // gets the RESOLVED model (gateways silently reroute — asking zen for `claude-zen-opus`
  // returns `gpt-5.6-sol@low`) and whether usage was actually reported.
  const contract = observeEnvelope({ requestedModel: model, body: data })

  return {
    text: textBlock.text,
    raw: data,
    truncation: describeTruncation(data, maxTokens),
    contract,
    /** The model that actually answered. Never assume this equals `model`. */
    resolvedModel: contract.resolvedModel,
  }
}
