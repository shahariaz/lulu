...
/**
 * Resolve the repository root that owns a task worktree.
 *
 * `git rev-parse --git-common-dir` returns the shared `.git` directory (a worktree's own gitdir
 * lives under `.git/worktrees/<id>`), so its parent is the repo root. Asked of git rather than
 * assuming the `<repo>/.zen-worktrees/<taskId>` convention, because the container mount layout
 * depends on this path being exactly right and a wrong guess fails obscurely.
 */
export function resolveRepoRootFromWorktree(worktreePath) {
...
export function integrateTaskCommit(repoPath, featureBranch, taskId, candidateCommitSha) {
  ...
  try {
    ...
    if (currentBranch === featureBranch) {
      ...
      // featureBranch is not checked out, so `git merge --ff-only` is unavailable and we move
      // the ref directly. `update-ref` does NOT enforce fast-forward — given a non-descendant
      // it silently resets the branch, discarding any commits already merged into it. So the
      // ancestry check below is the ONLY thing upholding the fast-forward invariant on this
      // path. Do not remove it, and do not let it fail open.
      const featureHead = runGit(resolvedRepo, ['rev-parse', featureBranch])

      let isAncestor = true
      try {
        // Exits 0 when featureHead is an ancestor of the candidate, 1 when it is not.
        // runGit throws on any non-zero exit.
        runGit(resolvedRepo, ['merge-base', '--is-ancestor', featureHead, candidateCommitSha])
      } catch {
        isAncestor = false
      }
      ...
      if (!isAncestor) {
        throw new Error(
          `${featureBranch} (${featureHead}) is not an ancestor of candidate ${candidateCommitSha} — ` +
          `it moved out-of-band. Rebase and re-verify the task before integrating.`
        )
      }

      // Third argument is the expected old value: a compare-and-swap that also rejects the
      // branch moving between the rev-parse above and this write.
