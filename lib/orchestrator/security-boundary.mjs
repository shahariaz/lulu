 ...
 */
export function sanitizeWorkerEnvironment(baseEnv = process.env, { allowEnv = [] } = {}) {
  const sanitized = {}
  // Named exceptions, opted into per spawn. The blanket rules below are correct for the WORKER —
  // untrusted agent code that must never see the owner's credentials — but they also strip
  // legitimate tool credentials from trusted helpers we invoke ourselves. BRAVE_API_KEY is the
  // case that found this: it matches the `KEY` rule, so setting it did nothing, silently, and
  // the search pool stayed at two engines while the owner believed they had three.
  //
  // An explicit list keeps the default closed. A caller must name the variable it needs, so
  // widening this is a visible decision at the call site rather than a hole in the boundary.
  const allowed = new Set(allowEnv)

  for (const [key, val] of Object.entries(baseEnv)) {
    // An explicitly-named secret (ANTHROPIC_API_KEY and friends) is never allowlistable —
    // routing around ANTHROPIC_BASE_URL would break the cost model that funds the whole system.
...
export function spawnControlledProcess(command, args, {
  ...
  onStdout = null,
  onStderr = null,
  /** Env vars this specific spawn is permitted to receive despite the blanket secret filter. */
  allowEnv = [],
} = {}) {
  const sanitizedEnv = sanitizeWorkerEnvironment(env, { allowEnv })
