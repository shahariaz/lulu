 ...
 */
/**
 * The model that actually served the worker's session, as reported by the CLI's system/init
 * event. Recorded rather than assumed: gateways silently reroute (asking the zen proxy for
 * `claude-zen-opus` returns `gpt-5.6-sol@low`), so the requested model is not evidence of which
 * model did the work — and the independent-review guarantee depends on knowing.
 */
export function resolvedModelFromEvents(events = []) {
...
export async function executeWorkerTask({
  ...
  const usage = aggregateUsageFromEvents(events)
  const resolvedModel = resolvedModelFromEvents(events)
  if (resolvedModel) {
    targetDb.prepare('UPDATE task_runs SET model = ? WHERE id = ?').run(resolvedModel, taskRun.id)
  }
